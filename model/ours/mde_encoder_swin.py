...
from .mde_model_utils import swin_convert
from .mde_model_utils import PatchEmbedSwin as PatchEmbed

from .mde_resnet import BasicBlock, Bottleneck
from .mde_model_utils import ResLayer
...
# {'type': 'DepthFormerSwin', 'pretrain_img_size': 224, 'embed_dims': 192, 'patch_size': 4, 'window_size': 7, 'mlp_ratio': 4, 'depths': [2, 2, 18, 2], 'num_heads': [6, 12, 24, 48], 'strides': (4, 2, 2, 2), 'out_indices': (0, 1, 2, 3), 'qkv_bias': True, 
# 'qk_scale': None, 'patch_norm': True, 'drop_rate': 0.0, 'attn_drop_rate': 0.0, 'drop_path_rate': 0.3, 'use_abs_pos_embed': False, 'act_cfg': {'type': 'GELU'}, 'norm_cfg': {'type': 'LN', 'requires_grad': True}, 'pretrain_style': 'official', 
# 'conv_norm_cfg': {'type': 'BN', 'requires_grad': True}, 'depth': 50, 'num_stages': 0}
    def __init__(self,
    ...
    def init_weights(self):
        ...
        # soft prompt
        # self.soft_prompt = nn.Parameter(torch.zeros(self.num_prompt, self.num_dim), requires_grad=True)
        # val = math.sqrt(6. / 2*self.num_dim)
        # nn.init.uniform_(self.soft_prompt.data, -val, val)
    ...
    def forward(self, x):
        # import pdb;pdb.set_trace()
        outs = []
        conv_stem = self.conv_stem(x) # B,3,H,W->B,64,H/2,W/2
        outs.append(conv_stem) # convolution 결과 저장

        x = self.patch_embed(x) # # B,3,H,W-> B, H/4*W/4, 192(emb_dim)

        hw_shape = (self.patch_embed.DH, self.patch_embed.DW) #  "# of patches"
        if self.use_abs_pos_embed:
            ...
            if i in self.out_indices:
                ...
                out = out.view(-1, *out_hw_shape,
                               self.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
                # B, H/4*W/4, emb_dim -> B, emb_dim, H/4, W/4
                outs.append(out)
